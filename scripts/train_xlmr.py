...
LOGGING_DIR = "./runs/xlmr_lora_crf_ner"
CHECKPOINT_DIR = "./trained_models/xlmr"
BEST_LORA_DIR = os.path.join(CHECKPOINT_DIR, "best_xlmr_lora_weights")
...
def plot_confusion_matrix(writer, cm, class_names, epoch=0):
    ...
    plt.title(
        "Confusion Matrix (XLM-RoBERTa + LoRA + CRF)",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
    ...
    save_path = os.path.join(CHECKPOINT_DIR, "xlmr_confusion_matrix.png")
...
class XLMRLoRACRF(nn.Module):
    def __init__(self, model_name, num_labels, pad_label_id=0):
        ...
        self.xlmr = get_peft_model(base_xlmr, peft_config)
        print("\nTHÔNG SỐ BỘ LORA THAM SỐ:")
        self.xlmr.print_trainable_parameters()

        hidden_size = base_xlmr.config.hidden_size
...
optimizer_grouped_parameters = [
    {
        "params": [
            p for n, p in model.xlmr.named_parameters() if p.requires_grad
        ],
        "lr": LR_LORA,
        "weight_decay": 0.01,
    },
...
for epoch in range(EPOCHS):
    ...
    torch.save(checkpoint, os.path.join(CHECKPOINT_DIR, "last_xlmr_lora.pt"))

    if val_f1 > best_f1:
        best_f1 = val_f1
        torch.save(checkpoint, os.path.join(CHECKPOINT_DIR, "best_xlmr_lora.pt"))
        model.xlmr.save_pretrained(BEST_LORA_DIR)
        tokenizer.save_pretrained(BEST_LORA_DIR)
    print("-" * 60)

print("\nEVALUATION RESULTS")
best_ckpt_path = os.path.join(CHECKPOINT_DIR, "best_xlmr_lora.pt")
...
print("\n--- REPORT (TEST SET - XLM-R + LORA + CRF) ---")
print(report)
...
sns.barplot(x="F1-Score", y="Entity", data=df_res, palette="rocket")
plt.title("XLM-RoBERTa + LoRA + CRF Performance on PhoNER_COVID19 Dataset")
plt.xlim(0, 1.0)
plt.tight_layout()
plt.savefig(os.path.join(CHECKPOINT_DIR, "phoner_covid19_xlmr_f1_scores.png"), dpi=300)
